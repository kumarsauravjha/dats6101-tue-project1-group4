...
train_indices_ <- sample(1:nrow(nfl_clean), 0.8 * nrow(nfl_clean))
...
```
Before concluding, in pursuit of modelling the dependent variable better, we wanted to try out one more model.So we employed the ensemble technique of Random Forest and re-modeled.
```{r Random_Forest}
