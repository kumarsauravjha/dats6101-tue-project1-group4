...
voilon_wt<-ggplot(nfl_data_clean, aes(x = weight, y = position_group, fill = weight)) +
  geom_violin() +
  labs(title = "Violin Plot of weight and position group")
print(voilon_wt)

voilon_ht<-ggplot(nfl_data_clean, aes(x = height, y = position_group, fill = height)) +
  geom_violin() +
...
# Using the same data as above
scatter_ht<-ggplot(nfl_data_clean, aes(x = position_group, y = height, color = position_group)) +
  geom_jitter() +
  labs(title = "Jittered Scatter Plot of position group by height")
print(scatter_ht)


scatter_wt<-ggplot(nfl_data_clean, aes(x = position_group, y = weight, color = position_group)) +
  geom_jitter() +
...
# Using the same data as above
strip_wt<-ggplot(nfl_data_clean, aes(x = position_group, y = weight, color = position_group)) +
  geom_point(position = position_jitter(width = 0.2), size = 3) +
  labs(title = "Strip Chart of position group by weight")
print(strip_wt)

strip_ht<-ggplot(nfl_data_clean, aes(x = position_group, y = height, color = position_group)) +
  geom_point(position = position_jitter(width = 0.2), size = 3) +
...
# Using the same data as above
faceted_wt<-ggplot(nfl_data_clean, aes(x = position_group, y = weight, color = position_group)) +
  geom_point() +
  facet_wrap(~position_group) +
  labs(title = "Faceted Scatter Plots of position group by weight")
print(faceted_wt)

faceted_ht<-ggplot(nfl_data_clean, aes(x = position_group, y = height, color = position_group)) +
  geom_point() +
  facet_wrap(~position_group) +
